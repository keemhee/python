...
cap = cv2.VideoCapture(0)  # 비디오 캡처 장치 초기화
...
while True:
    ...
    if contour:
        c_x = x + int(w / 2)
        c_y = y + int(h / 2)
        points.append((c_x, c_y))  # 중심점을 리스트에 추가
        for i in range(1, len(points)):  # 첫 번째 점을 제외한 모든 점들을 순회
            cv2.line(frame, points[i - 1], points[i], (0, 255, 255), 3)  # 이전 점과 현재 점을 연결하는 선을 그림
